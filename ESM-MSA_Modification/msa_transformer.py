    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if need_head_weights:
                ...
                #print(col_attn.permute(2, 0, 1, 3, 4).size())
                # H x B x C x C -> B x H x C x C
                row_attn_weights.append(row_attn.permute(1, 0, 2, 3))
                #print(row_attn.permute(1, 0, 2, 3).size())
            if (layer_idx + 1) in repr_layers:
                #print(x.permute(2, 0, 1, 3).size())
        ...
        if need_head_weights:
            ...
            #print(col_attentions.size())
            # row_attentions: B x L x H x C x C
            row_attentions = torch.stack(row_attn_weights, 1)
            #print(row_attentions.size())
            #result["col_attentions"] = col_attentions
            result["row_attentions"] = row_attentions
            if return_contacts:
                contacts, sys_attention_map, last_layer_attentions = self.contact_head(tokens, row_attentions)
                #print(contacts.size())
                result["contacts"] = contacts
                result["sys_attentions"] = sys_attention_map
                result["last_layer_attentions"] = last_layer_attentions
